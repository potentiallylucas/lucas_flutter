...
class ConnectionState {
  Socket? theServer; // Socket.  The Socket for client and
...
class SaidState {
  final List<String> messages; // Store multiple messages
...
class SaidCubit extends Cubit<SaidState> {
  ...
  void update(String message) {
    ...
    emit(SaidState(updatedMessages));
  }
}
          ...
          cs.theServer != null
              ? Expanded(
                child: ListView.builder(
                  itemCount: sc.state.messages.length,
                  itemBuilder: (context, index) {
                    return ListTile(title: Text(sc.state.messages[index]));
                  },
                ),
              )
