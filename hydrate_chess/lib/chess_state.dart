...
class ChessState
{
  ...
  factory ChessState.fromJson(Map<String, dynamic> json) { // takes json and turns it into board data and turncount int
    List<List<String>> board = (json['board'] as List)
      .map((column) => (column as List).map((cell) => cell as String).toList())
      .toList();
    int turnCount = json['turnCount'] as int;
    return ChessState.load(board, turnCount);
  }

  Map<String, dynamic> toJson() { //turns board data and turncount to json
    return { //
      'board': board,
      'turnCount': turnCount,
    };
